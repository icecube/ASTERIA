...
if MODE == "RUN":
    ...
    filename = "./files/scan/{}/{}/SCAN_model_{}_{:.0f}_mode_{}_time_{:.0f}ms-{:.0f}ms_mix_{}_hier_{}_sig_trials_{:1.0e}_bkg_trials_{:1.0e}_ampl_{:.1f}%.npz".format(
        ...
        sig_trials, bkg_trials, amplitude[0]*100)
    scan.save(filename = filename)

elif MODE == "COMBINE":
    # COMBINE DATA
    #filebase = "./files/scan/{}/{}/SCAN_model_{}_{:.0f}_mode_{}_time_{:.0f}ms-{:.0f}ms_mix_{}_hier_{}_sig_trials_{:1.0e}_bkg_trials_{:1.0e}".format(
    #    ft_mode, scan_dir_name, model["name"], model["param"]["progenitor_mass"].value, 
    #    ft_mode, time_start.value, time_end.value, mixing_scheme, hierarchy,
    #    sig_trials, bkg_trials)
    #scan.combine(filebase = filebase, ampl_range = ampl_range, item = "dist")

    filename = "./files/scan/{}/{}/SIGN_model_{}_{:.0f}_mode_{}_time_{:.0f}ms-{:.0f}ms_mix_{}_hier_{}_sig_trials_{:1.0e}_bkg_trials_{:1.0e}".format(
        ft_mode, scan_dir_name, model["name"], model["param"]["progenitor_mass"].value, 
        ft_mode, time_start.value, time_end.value, mixing_scheme, hierarchy,
        sig_trials, bkg_trials) +"_ampl_{:.1f}-{:.1f}%.npz".format(
